<?php // customer_testimonials infobox 

/*** Scrolling Customer Testimonials v1.0 ***/
...
// create Write Testimonial link, if applicable
		...
		if (((TESTIMONIAL_REGISTERED_WRITE == "Registered Users Only") && (tep_session_is_registered('customer_id'))) || (TESTIMONIAL_REGISTERED_WRITE == "All Users")) {
			$testimonial_write_link = tep_draw_separator('h_gray_middle.gif', '100%', '3') . '<center><a href="' . tep_href_link(FILENAME_CUSTOMER_TESTIMONIALS_WRITE, tep_get_all_get_params(array('language', 'currency'))) . '">' . IMAGE_BUTTON_WRITE_TESTIMONIAL . '</a></center>';
		}
...
// check language display preference
	if (TESTIMONIAL_LANG_PREF == "User Language Only") {  // display only localized testimonials?
// Count how many testimonials in database that match user's language
	...
	$testimonials_new_query = tep_db_query("select * FROM " . TABLE_CUSTOMER_TESTIMONIALS . " WHERE status = 1 ORDER BY rand() LIMIT " . $testimonials_limit); 
	}
...
// Begin infobox heading
  ...
  $boxLink = '<a href="' . tep_href_link(FILENAME_CUSTOMER_TESTIMONIALS) . '"><img src="images/infobox/arrow_right_3.gif" border="0" alt="more" title="more" width="12" height="10"></a>';
// End infobox heading
...
/*** Begin Scrolling Customer Testimonials v1.1 ***/
// check language display preference
	if (TESTIMONIAL_LANG_PREF == "User Language Only") {  // pick only a localized testimonial?
