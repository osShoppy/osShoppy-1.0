<?php // scrolling_whats_new infobox

if(EXCLUDE_GIFT_PRODUCTS_NEW == 'true') {
  if (!empty($hiddencats)) {
    $rp_query = tep_db_query( "select p.products_id, products_image, products_tax_class_id, products_price, pd.products_name from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where products_carrot = '0' and products_status = '1' and p.products_id = p2c.products_id and (not (p2c.categories_id in (" . implode(',', $hiddencats) . "))) group by p.products_id order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW);
  } else {
    $rp_query = tep_db_query( "select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where products_carrot = '0' and products_status = '1' and quotes_email_address NOT LIKE '%@%' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW);
  }
} else {
  if (!empty($hiddencats)) {
    ...
    $rp_query = tep_db_query( "select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where products_status = '1' and quotes_email_address NOT LIKE '%@%' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW);
  }
}
...
$scroller = "<script type='text/javascript'>new pausescroller(pausecontent_np, 'pscroller_np', 'pscroller_c_np', 2000);</script>";
  ...
  $box_base_name = 'scrolling_whats_new';
