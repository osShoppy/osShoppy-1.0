...
require(DIR_FS_CATALOG . DIR_WS_LANGUAGES . $language . '/modules/html_mail/basic/edit_orders.php');
	...
	<table width="600" border="0" align="center" background="'. HTTP_SERVER . DIR_WS_CATALOG . DIR_WS_MODULES .'html_mail/'.HTML_EMAIL_LAYOUT.'/images/checkout_process/background_mid.png" cellpadding="0" cellspacing="0">
		...
		<td>
			<table width="100%" border="0" align="center">
		  		<tr>
		    		<td><font face="Tahoma, Times New Roman, Times"><span style="font-size:14px;"><span style="font-size: 24px;">'.UHE_TEXT_DEAR . ' ' . $order->customer['name'] . ',</span><br/>'.UHE_MESSAGE_GREETING.'</span></font></td>
		  		</tr>
			</table>
			<table width="100%" border="0" align="center">
		    	<td width="200"><font face="Tahoma, Times New Roman, Times" style="font-size:14px;"><strong>'.UHE_TEXT_ORDER_NUMBER.'</strong></font></td>
                <td><font face="Tahoma, Times New Roman, Times" style="font-size:14px;">&nbsp;'.(int)$oID.'</font></td>
			</table>
			<table width="100%" border="0" align="center">
		    	<td width="200"><font face="Tahoma, Times New Roman, Times" style="font-size:14px;"><strong>'.UHE_TEXT_INVOICE_URL.'</strong></font></td>
                <td><font face="Tahoma, Times New Roman, Times" style="font-size:14px;">&nbsp;<a href="' . tep_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_id=' . (int)$oID, 'SSL', false) .'">' . tep_href_link(FILENAME_CATALOG_ACCOUNT_HISTORY_INFO, 'order_id=' . (int)$oID, 'SSL', false) .'</a></font></td>
			</table>
			<table width="100%" border="0" align="center">
		...
		<table style="font-size:14px; font-family:\'Tahoma, Times New Roman, Times\';" border="0" cellpadding="3" cellspacing="2">
			...
			<tr style="background-color:#DDDDDD;">';
				if ($order->content_type != 'virtual') {
					$html_email .= '<td><font face="Tahoma, Times New Roman, Times" style="font-size:14px;">'. $order->delivery['name'] . '<br />' . $order->delivery['company'] . '<br />' . $order->delivery['suburb'] . '<br />' . $order->delivery['street_address'] . '<br />' . $order->customer['postcode'] . ' ' . $order->customer['city'] . '<br />' . $order->delivery['state'] . '<br />' . $order->delivery['country'] . '</font></td>';
				}
					$html_email .= '<td><font face="Tahoma, Times New Roman, Times" style="font-size:14px;">'. $order->billing['name'] . '<br />' . $order->billing['company'] . '<br />' . $order->billing['suburb'] . '<br />' . $order->billing['street_address'] . '<br />' . $order->customer['postcode'] . ' ' . $order->customer['city'] . '<br />' . $order->billing['state'] . '<br />' . $order->billing['country'] . '<br />' . $order->info['payment_method'] . '</font></td>
			</tr>
		</table>
		';
