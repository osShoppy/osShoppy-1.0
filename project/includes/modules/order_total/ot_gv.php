     ...
     global $currencies;
       ...
       if ($my_od_amount > 0) {
         $this->my_output[] = array('title' => $this->title . ':',
                          'text' => '- ' . $currencies->format($my_od_amount) . '',
                          'value' => $my_od_amount);
       }
   }
...
// END added by Rigadin                
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_GV_SORT_ORDER', '13', 'Sort order of display.', '6', '2', now())");
