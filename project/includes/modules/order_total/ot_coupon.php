...
global $PHP_SELF, $order, $currencies;
  ...
  if ($od_amount > 0) {
    ...
    $this->output[] = array('title' => $this->title . '' . $this->coupon_code .'</font>):','text' => '- ' . $currencies->format($od_amount) . '', 'value' => $od_amount); //Fred added hyphen
  }
}
...
function credit_selection() {
  ...
  $selection_string .= TEXT_ENTER_GV_CODE . tep_draw_input_field('gv_redeem_code', TEXT_REDEEM_CODE) ;
  $selection_string .= ' ';
...
//      $err_msg = ERROR_REDEEMED_AMOUNT.$coupon_amount_out;
        $err_msg = ERROR_REDEEMED_AMOUNT; // COMMENTED OUT for error handle reguest. Valid coupon did not return on checkout_payment.php (09-12-2012) by Gerard
    }
...
function install() {
  ...
  tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_ORDER_TOTAL_COUPON_SORT_ORDER', '7', 'Sort order of display.', '6', '2', now())");
