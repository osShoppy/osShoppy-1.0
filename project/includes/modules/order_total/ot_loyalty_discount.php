<?php // catalog/includes/modules/order_total/ot_loyalty_discount.php (1286)
...
// round discount to nearest cent. Discount of less than .5 cent will not be deducted from amount payable.
	...
	if ($od_amount>0) { // deduct discount from amount payable
		...
		$this->output[] = array('title' => $this->title . ':<br>' . MODULE_LOYALTY_DISCOUNT_SPENT . $currencies->format($this->cum_order_total) . $this->period_string . MODULE_LOYALTY_DISCOUNT_QUALIFY . $this->od_pc . '%</b>:',
								'text' => '- ' . $currencies->format($od_amount) .'<b>' ,
								'value' => $od_amount);
...
// Check if gift voucher is in cart and adjust total
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_LOYALTY_DISCOUNT_SORT_ORDER', '8', 'Sort order of display.', '6', '2', now())");
