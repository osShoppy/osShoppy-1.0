<?php // recently_viewed_column infobox 
...
// Retrieve the data on the products in the recently viewed list and load into an array by products_id
    ...
    if (count ($recently_viewed_array) > 0) { // Show only if we still have products in the array
      ...
      if (RECENTLY_VIEWED_BOX_SHOW_BUTTON == 'True') {
        ...
        $info_box_text .= '<a href="' . tep_href_link (FILENAME_RECENTLY_VIEWED) . '">' . tep_image_button ('button_more.png', IMAGE_BUTTON_SEE_MORE) . '</a>';
...
<?php
  ...
  $box_base_name = 'recently_viewed_column'; // for easy unique box template setup (added BTSv1.2)
