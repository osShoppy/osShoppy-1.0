<?php // whats_new_column infobox 

// activate inactivate category /Step-1(1)\
//  if ($random_product = tep_random_select("select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where products_status = '1' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW)) {
if(EXCLUDE_GIFT_PRODUCTS_NEW == 'true') {
  if (!empty($hiddencats)) {
    $new_query_raw = "select p.products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_carrot = '0' and products_status = '1' and p.products_id = p2c.products_id and (not (p2c.categories_id in (" . implode(',', $hiddencats) . "))) group by p.products_id order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW;
  } else {
    $new_query_raw = "select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where p.products_carrot = '0' and products_status = '1' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW;
  }
} else {
  if (!empty($hiddencats)) {
    ...
    $new_query_raw = "select products_id, products_image, products_tax_class_id, products_price from " . TABLE_PRODUCTS . " where products_status = '1' order by products_date_added desc limit " . MAX_RANDOM_SELECT_NEW;
  }
}
  ...
  $box_base_name = 'whats_new_column'; // for easy unique box template setup (added BTSv1.2)
