<?php // catalog/includes/functions/links.php (5272)
...
// TABLES: links_to_link_categories
  ...
  function tep_get_parent_category($catID = '0') {
    ...
    $categories_query = tep_db_query("select c.parent_id from " . TABLE_LINK_CATEGORIES . " c left join " . TABLE_LINK_CATEGORIES_DESCRIPTION . " cd on c.link_categories_id = cd.link_categories_id where c.link_categories_id = '" . (int)$catID . "' and cd.language_id = '" . (int)$languages_id . "' LIMIT 1");
...
// The HTML image wrapper function
  function tep_links_image($src, $alt = '', $width = '', $height = '', $parameters = '') {
    ...
    if (strpos($src, 'http') === FALSE) {
        $src = DIR_WS_IMAGES . 'uploads_links_manager/' . $src;
    }
...
***************************************************************************/
function NoParentLink($links_url, $links_reciprocal_url)
{
    $mainDomain = GetDomainName($links_url);
    $linkArray = parse_url($links_reciprocal_url);
...
////
// Try to connect to the site to verify it actually exists
function NotValidURL($url)
{
    return (tep_not_null(GetLinksFileArray($url)) ? false : true);
}

function tep_get_child_categories($parentID) {
    global $languages_id;
    $categories_query = tep_db_query("select lc.link_categories_id, lcd.link_categories_name, lcd.link_categories_description, lc.link_categories_image, lc.parent_id from " . TABLE_LINK_CATEGORIES . " lc left join " . TABLE_LINK_CATEGORIES_DESCRIPTION . " lcd on lc.link_categories_id = lcd.link_categories_id where lc.link_categories_status = '1' and lc.parent_id = '" . (int)$parentID  . "' and lcd.language_id = '" . (int)$languages_id . "' order by lc.link_categories_sort_order, lcd.link_categories_name");
...
// TABLES: categories
  ...
  function tep_sanitize_url_string($string) {
