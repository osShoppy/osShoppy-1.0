<?php //$Id: /catalog/admin/returns.php (1136)
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'update_order':
      ...
      if ($check_status['returns_status'] != $status) {
		...
		$notify_status = ($HTTP_POST_VARS['notify'] == 'on') ? 1 : 0;
    ...
    if (tep_db_num_rows($returns_history_query)) {
      while ($returns_history = tep_db_fetch_array($returns_history_query)) {
        ...
        if ($returns_history['customer_notified'] == '1') {
          echo tep_image(DIR_WS_ICONS . 'file_icons/tick.gif', ICON_TICK) . "</td>\n";
        } else {
          echo tep_image(DIR_WS_ICONS . 'file_icons/cross.gif', ICON_CROSS) . "</td>\n";
        }
        echo '            <td class="smallText">' . $orders_status_array[$returns_history['returns_status']] . '</td>' . "\n" .
             ...
             '          </tr>' . "\n";
			  ...
			  echo '          <tr>' . "\n" .
			       ...
			       '          <td class="smallText">' . tep_image(DIR_WS_ICONS . 'file_icons/cross.gif', ICON_CROSS) . "</td>\n" .
...
<tr>
  <td colspan="2" align="right"><?php echo
				...
				'<a href="' . tep_href_link(FILENAME_RETURNS_INVOICE, 'oID=' . $HTTP_GET_VARS['oID']) . '" TARGET="_blank">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a> <a href="' . tep_href_link("returns_packingslip.php", 'oID=' . $HTTP_GET_VARS['oID']) . '" TARGET="_blank">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a> <a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('action'))) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>'; ?></td>
</tr>
<?php
...
?>
            ...
            <td class="dataTableContent" align="center"><b><?php echo $orders['rma_value']; ?></b></a>
                <?php // END BURT ?>
              <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_RETURNS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders['returns_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'file_icons/preview.gif', ICON_PREVIEW) . '&nbsp;' . $order_done . $return_customer_name . '</font></a>' ; ?></td>
...
?>
            <tr>
              <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $orders_split->display_count($orders_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_ORDERS_RETURN); ?></td>
