<?php //$Id: /catalog/admin/documents.php (7005)
...
// Set a document type Visible
      case 'process_doc_type_visible':
        if ( ($_GET['flag'] == 'False') || ($_GET['flag'] == 'True') ) {
          if (isset ($_GET['dt']) ) {
            $query = "update " . TABLE_DOCUMENT_TYPES . "
        ...
        tep_redirect (tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','flag','dt') ) ) );
        break;
...
//          print $type_query_raw . "<br>\n";
        ...
        tep_redirect (tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','dt') ) . '&dt=' . $new_document_types_id) );
        break;
...
// Edit a document type
      case 'process_edit_type':
        if (isset ($_POST['document_types_id']) && $_POST['document_types_id'] > 0) {
          ...
          tep_db_perform (TABLE_DOCUMENT_TYPES, $sql_data_array, 'update', "document_types_id ='" . (int) $_POST['document_types_id'] . "'");
        } // if (isset
          
        tep_redirect (tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','dt') ) . 'dt=' . $doc_type) );
        break;
...
// Set documents Visible
      case 'doc_visible_confirm':
        ...
        tep_redirect (tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action') ) ) );
        break;

//Complete the delete process. Remove the file and all database entries.
      case 'doc_delete_confirm':
        tep_remove(DIR_FS_CATALOG_DOCUMENTS . $_POST['type'] . '/' . $_POST['name']);
        tep_db_query("delete from " . TABLE_DOCUMENTS . "
                      where documents_id = '" . (int)$_GET['did'] . "'
                    ");
        tep_db_query("delete from " . TABLE_PRODUCTS_TO_DOCUMENTS . "
        ...
        tep_redirect (tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . $error) );
        break;
...
//Find all of the files in the directory that do not have database entries and add them.
      case 'process_doc_update':
        if ($doc_path != 0) {
          ...
          $type = tep_db_fetch_array ($type_query);
          $dir_path = (string) (DIR_FS_CATALOG_DOCUMENTS . $type['type_name']);

          if (is_dir ($dir_path) ) {
            $add_files = get_directory_list ($dir_path);
          }
...
//          print $documents_query_raw . "<br>\n";
          ...
          if (count ($add_files) > 0) {
            foreach ($add_files as $filename) {
              ...
              tep_db_perform (TABLE_DOCUMENTS, $sql_data_array);
            } // foreach
          } // if (count
        } // if ($doc_path
        
        tep_redirect (tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) ) );
        break;
...
//Save data to the database after an Edit operation
      case 'doc_edit_confirm':
        if (isset ($_POST['type']) && isset ($_POST['name']) ) {
          ...
          $file_path = DIR_FS_CATALOG_DOCUMENTS . $type['type_name'] . '/' . $documents_name;
          ...
          if (file_exists ($file_path) && filesize ($file_path) > 0) {
            ...
            tep_db_perform (TABLE_DOCUMENTS, $sql_data_array, 'update', "documents_id ='" . (int) $_GET['did'] . "'");
          } // if (file_exists
        } // if (isset ($_POST['type']
        
        tep_redirect(tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $_GET['did']));
        break;
...
//Upload a file and update the database
      case 'process_doc_upload':
        if (tep_not_null ($_POST['document_types_id']) ) {
          ...
          $document_upload = new upload ('documents_name');
          $document_upload->set_destination (DIR_FS_CATALOG_DOCUMENTS . $type_name);

          if ($document_upload->parse() && $document_upload->save() ) {
            $documents_name = $document_upload->filename;
            $documents_path = DIR_FS_CATALOG_DOCUMENTS . $type_name . '/' . $documents_name;
        ...
        tep_redirect(tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did')) . 'did=' . $insert_id));
        break;
...
// Associate a document with a product
      case 'doc_associate_confirm':
        if (tep_not_null($_GET['did']) && tep_not_null($_POST['product']) && ($_POST['product'] != '-')) {
          tep_db_query("insert into " . TABLE_PRODUCTS_TO_DOCUMENTS . " (
                                     products_id,
                                     documents_id,
                                     last_updated                      )
...
<?php
    ...
    while ($document_types = tep_db_fetch_array ($document_types_query) ) {
      ...
      if (isset ($dtInfo) && is_object ($dtInfo) && ($document_types['document_types_id'] == $dtInfo->document_types_id) ) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link (FILENAME_DOCUMENTS, 'dt=' . $document_types['document_types_id']) . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link (FILENAME_DOCUMENTS, 'dt=' . $document_types['document_types_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link (FILENAME_DOCUMENTS, 'dtpath=' . $document_types['document_types_id']) . '">' . tep_image (DIR_WS_ICONS . 'file_icons/folder.gif', ICON_FOLDER) . '</a>&nbsp;<b>' . $document_types['type_description'] . '</b>'; ?></td>
                <td class="dataTableContent"><?php echo $document_types['type_name']; ?></td>
                <td class="dataTableContent" align="center">
<?php
      if ($document_types['type_visible'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icons/status_icons/icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link (FILENAME_DOCUMENTS, 'action=process_doc_type_visible&flag=False&dt=' . $document_types['document_types_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icons/status_icons/icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link (FILENAME_DOCUMENTS, 'action=process_doc_type_visible&flag=True&dt=' . $document_types['document_types_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icons/status_icons/icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image (DIR_WS_IMAGES . 'icons/status_icons/icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?>
                </td>
...
<?php 
        if (isset ($cInfo) && is_object ($cInfo) && ($document_types['document_types_id'] == $cInfo->document_types_id) ) { 
          echo tep_image (DIR_WS_IMAGES . 'icons/info_icons/icon_arrow_right.gif', ''); 
        } else { 
          echo '<a href="' . tep_href_link (FILENAME_DOCUMENTS, 'dt=' . $doc_type) . '">' . tep_image (DIR_WS_IMAGES . 'icons/info_icons/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; 
        } 
?>
                &nbsp;</td>
              </tr>
              ...
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php echo '<a href="' . tep_href_link (FILENAME_DOCUMENTS, 'action=new_type') . '">' . tep_image_button ('button_new_document_type.png', IMAGE_NEW_DOCUMENT_TYPE) . '</a>'; ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
...
<?php
  $documents_query_raw = "select documents_id,
    ...
    if (isset ($dInfo) && is_object($dInfo) && $documents['documents_id'] == $dInfo->documents_id) {
      echo '                  <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $lInfo->documents_id . '&action=edit') . '\'">' . "\n";
    } else {
      echo '                  <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $documents['documents_id']) . '\'">' . "\n";
    }
...
?>
                <td class="dataTableContent"><?php echo $documents['documents_name']; ?></td>
...
<?php
      if ($documents['documents_visible'] == 'True') {
        echo tep_image (DIR_WS_IMAGES . 'icons/status_icons/icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','flag','did') ) . 'action=doc_visible_confirm&flag=False&did=' . $documents['documents_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','flag','did') ) . 'action=doc_visible_confirm&flag=True&did=' . $documents['documents_id']) . '">' . tep_image (DIR_WS_IMAGES . 'icons/status_icons/icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }

?>
                </td>
                <td class="dataTableContent" align="right"><?php if (isset($dInfo) && is_object($dInfo) && ($documents['documents_id'] == $dInfo->documents_id)) { echo tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('did') ) . 'did=' . $documents['documents_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    } // while
?>
              <tr>
                <td colspan=6><table border="0" width="100%" cellspacing="0" cellpadding="2">
                 ...
                 <tr>
                  ...
                  <tr>
                    <td>&nbsp;</td>
                    <td align="right"><?php echo '<a href="' . tep_href_link (FILENAME_DOCUMENTS, 'dt=' . $doc_path) . '">' . tep_image_button ('button_back.png', IMAGE_BACK) . '</a> <a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action') ) . '&action=upload') . '">' . tep_image_button('button_upload.png', IMAGE_UPLOAD) . '</a> <a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . '&action=process_doc_update') . '">' . tep_image_button('button_update.png', IMAGE_UPDATE) . '</a>'; ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
  ...
  switch ($action) {
    ...
    case 'new_type':
      ...
      $contents = array ('form' => tep_draw_form ('new_type', FILENAME_DOCUMENTS, 'action=process_new_type', 'post'));
      $contents[] = array ('text' => TEXT_INFO_NEW_TYPE_INTRO);
      
      $directory_array = get_directory_list (DIR_FS_CATALOG_DOCUMENTS, false);
      ...
      $contents[] = array ('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.png', IMAGE_UPDATE) . ' <a href="' . tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $dInfo->document_types_id) . '">' . tep_image_button ('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;

    case 'edit_type':
      $heading[] = array ('text' => '<b>' . TEXT_INFO_HEADING_EDIT_DOC_TYPE . '</b>');

      $contents = array ('form' => tep_draw_form ('edit_type', FILENAME_DOCUMENTS, 'dt=' . $dtInfo->document_types_id . '&action=process_edit_type', 'post'));
      ...
      $contents[] = array ('align' => 'center', 'text' => '<br>' . tep_image_submit ('button_update.png', IMAGE_UPDATE) . ' <a href="' . tep_href_link (FILENAME_DOCUMENTS, 'dt=' . $dtInfo->document_types_id) . '">' . tep_image_button ('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;

    case 'delete_type':
      $heading[] = array ('text' => '<b>' . TEXT_INFO_HEADING_DELETE_DOC_TYPE . '</b>');

      $contents = array ('form' => tep_draw_form ('delete_type', FILENAME_DOCUMENTS, 'dt=' . $dtInfo->document_types_id . '&action=process_delete_type', 'post'));
      ...
      $contents[] = array ('align' => 'center', 'text' => '<br>' . tep_image_submit ('button_confirm.png', IMAGE_CONFIRM) . ' <a href="' . tep_href_link (FILENAME_DOCUMENTS, 'dt=' . $dtInfo->document_types_id) . '">' . tep_image_button ('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    ...
    case 'upload':
      ...
      $contents = array ('form' => tep_draw_form ('file_upload', FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action') ) . 'action=process_doc_upload', 'post', 'enctype="multipart/form-data"') );
      ...
      $contents[] = array ('align' => 'center', 'text' => '<br>' . tep_image_submit ('button_upload.png', IMAGE_UPLOAD) . ' <a href="' . tep_href_link (FILENAME_DOCUMENTS, (isset($_GET['info']) ? 'info=' . urlencode($_GET['info']) : '')) . '">' . tep_image_button ('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;

    case 'update':
      $heading[] = array ('text' => '<b>' . TEXT_INFO_HEADING_UPDATE . '</b>');

      $contents = array ('form' => tep_draw_form ('file_update', FILENAME_DOCUMENTS, 'action=process_doc_update', 'post'));
      ...
      $contents[] = array ('align' => 'center', 'text' => '<br>' . tep_image_submit ('button_update.png', IMAGE_UPDATE) . ' <a href="' . tep_href_link (FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $dInfo->documents_id) . '">' . tep_image_button ('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;

    case 'edit':
      $heading[] = array ('text' => '<b>' . TEXT_INFO_HEADING_EDIT_DOCUMENT . '</b>');

      $contents = array ('form' => tep_draw_form ('file', FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $dInfo->documents_id . '&action=doc_edit_confirm', 'post'));
    ...
    case 'associate':
      ...
      $contents = array('form' => tep_draw_form ('file', FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $dInfo->documents_id . '&action=doc_associate_confirm', 'post'));
    ...
    case 'delete':
      ...
      $contents = array('form' => tep_draw_form ('file', FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . '&did=' . $dInfo->documents_id . '&action=doc_delete_confirm', 'post'));
      ...
      $contents[] = array('text' => tep_draw_hidden_field('name', $dInfo->documents_name));
      $contents[] = array('text' => tep_draw_hidden_field('type', $dInfo->document_types_id));
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_confirm.png', IMAGE_CONFIRM) . ' <a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','flag','did') ) . 'did=' . $dInfo->documents_id) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    ...
    default:
      if ($doc_path == 0) {
        ...
        if (isset ($dtInfo->document_types_id) ) {
          $contents[] = array ('align' => 'center', 'text' => '<a href="' . tep_href_link (FILENAME_DOCUMENTS, 'action=edit_type&dt=' . $dtInfo->document_types_id) . '">' . tep_image_button ('button_edit.png', IMAGE_EDIT) . '</a>  <a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','dt') ) . 'dt=' . $dtInfo->document_types_id . '&action=delete_type') . '">' . tep_image_button ('button_delete.png', IMAGE_DELETE) . '</a>');
        }
        ...
        $heading[] = array ('text' => '<b>' . TEXT_INFO_HEADING_GENERAL . $documents['type_description'] . '</b>');

        if (isset ($dInfo->documents_id) ) {
          $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . '&action=edit&did=' . $dInfo->documents_id) . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a>  <a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','did') ) . 'did=' . $dInfo->documents_id . '&action=associate') . '">' . tep_image_button('button_associate.png', IMAGE_ASSOCIATE) . '</a> <a href="' . tep_href_link(FILENAME_DOCUMENTS, tep_get_all_get_params (array ('action','flag','did') ) . 'did=' . $dInfo->documents_id . '&action=delete') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>');
