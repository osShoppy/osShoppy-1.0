...
*/
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'insert':
    case 'save':
      $ticket_status_id = tep_db_prepare_input($HTTP_GET_VARS['oID']);

      $languages = tep_get_languages();
      $ticket_status_name_array = $HTTP_POST_VARS['ticket_status_name'];
      ...
      tep_redirect(tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $ticket_status_id));
      break;
    case 'deleteconfirm':
      ...
      tep_redirect(tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page']));
      break;
    case 'delete':
      $oID = tep_db_prepare_input($HTTP_GET_VARS['oID']);
...
<?php
  ...
  $ticket_status_split = new splitPageResults($HTTP_GET_VARS['page'], MAX_DISPLAY_SEARCH_RESULTS, $ticket_status_query_raw, $ticket_status_query_numrows);
  $ticket_status_query = tep_db_query($ticket_status_query_raw);
  while ($ticket_status = tep_db_fetch_array($ticket_status_query)) {
    if (((!$HTTP_GET_VARS['oID']) || ($HTTP_GET_VARS['oID'] == $ticket_status['ticket_status_id'])) && (!$oInfo) && (substr($HTTP_GET_VARS['action'], 0, 3) != 'new')) {
      $oInfo = new objectInfo($ticket_status);
    }

    if ( (is_object($oInfo)) && ($ticket_status['ticket_status_id'] == $oInfo->ticket_status_id) ) {
      echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->ticket_status_id . '&action=edit') . '\'">' . "\n";
    } else {
      echo '                  <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $ticket_status['ticket_status_id']) . '\'">' . "\n";
    }
...
?>
                <td class="dataTableContent" align="right"><?php if ( (is_object($oInfo)) && ($ticket_status['ticket_status_id'] == $oInfo->ticket_status_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $ticket_status['ticket_status_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
              <tr>
                <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $ticket_status_split->display_count($ticket_status_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_TEXT_STATUS); ?></td>
                    <td class="smallText" align="right"><?php echo $ticket_status_split->display_links($ticket_status_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $HTTP_GET_VARS['page']); ?></td>
                  </tr>
<?php
  if (substr($HTTP_GET_VARS['action'], 0, 3) != 'new') {
?>
                  <tr>
                    <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&action=new') . '">' . tep_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'new':
      ...
      $contents = array('form' => tep_draw_form('status', FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&action=insert'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.gif', IMAGE_INSERT) . ' <a href="' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'edit':
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_EDIT_TEXT_STATUS . '</b>');

      $contents = array('form' => tep_draw_form('status', FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->ticket_status_id  . '&action=save'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->ticket_status_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    case 'delete':
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DELETE_TEXT_STATUS . '</b>');

      $contents = array('form' => tep_draw_form('status', FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->ticket_status_id  . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
      $contents[] = array('text' => '<br><b>' . $oInfo->ticket_status_name . '</b>');
      if ($remove_status) $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->ticket_status_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (is_object($oInfo)) {
        $heading[] = array('text' => '<b>' . $oInfo->ticket_status_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->ticket_status_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_TICKET_STATUS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->ticket_status_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
