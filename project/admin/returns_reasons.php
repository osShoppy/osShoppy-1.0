<?php //$Id: /catalog/admin/returns_reasons.php (1136)
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'insert':
    case 'save':
      $return_reason_id = tep_db_prepare_input($HTTP_GET_VARS['oID']);

      $languages = tep_get_languages();
      for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
        $return_reason_name_array = $HTTP_POST_VARS['return_reason_name'];
      ...
      tep_redirect(tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $return_reason_id));
      break;
    case 'deleteconfirm':
      ...
      tep_redirect(tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page']));
      break;
    case 'delete':
      $oID = tep_db_prepare_input($HTTP_GET_VARS['oID']);
       /*
...
<?php
  ...
  while ($orders_status = tep_db_fetch_array($orders_status_query)) {
    if (((!$HTTP_GET_VARS['oID']) || ($HTTP_GET_VARS['oID'] == $orders_status['return_reason_id'])) && (!$oInfo) && (substr($HTTP_GET_VARS['action'], 0, 3) != 'new')) {
      $oInfo = new objectInfo($orders_status);
    }

    if ( (is_object($oInfo)) && ($orders_status['return_reason_id'] == $oInfo->return_reason_id) ) {
      echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->return_reason_id . '&action=edit') . '\'">' . "\n";
    } else {
      echo '                  <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $orders_status['return_reason_id']) . '\'">' . "\n";
    }
...
?>
                <td class="dataTableContent" align="right"><?php if ( (is_object($oInfo)) && ($orders_status['return_reason_id'] == $oInfo->return_reason_id) ) { echo tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $orders_status['return_reason_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
              <tr>
                <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $orders_status_split->display_count($orders_status_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_RETURN_REASON); ?></td>
...
?>
                  <tr>
                    <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&action=new') . '">' . tep_image_button('button_insert.png', IMAGE_INSERT) . '</a>'; ?></td>
                  </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'new':
      ...
      $contents = array('form' => tep_draw_form('status', FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&action=insert'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_insert.png', IMAGE_INSERT) . ' <a href="' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page']) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    case 'edit':
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_EDIT_ORDERS_STATUS . '</b>');

      $contents = array('form' => tep_draw_form('status', FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->return_reason_id  . '&action=save'));
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_update.png', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->return_reason_id) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    case 'delete':
      $heading[] = array('text' => '<b>' . TEXT_INFO_HEADING_DELETE_ORDERS_STATUS . '</b>');

      $contents = array('form' => tep_draw_form('status', FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->return_reason_id  . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
      $contents[] = array('text' => '<br><b>' . $oInfo->return_reason_name . '</b>');
      if ($remove_status) $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->return_reason_id) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (is_object($oInfo)) {
        $heading[] = array('text' => '<b>' . $oInfo->return_reason_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->return_reason_id . '&action=edit') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_RETURNS_REASONS, 'page=' . $HTTP_GET_VARS['page'] . '&oID=' . $oInfo->return_reason_id . '&action=delete') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>');
