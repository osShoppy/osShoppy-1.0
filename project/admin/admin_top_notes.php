<?php //$Id: /catalog/admin/admin_top_notes.php (2599)

  require('includes/application_top.php');

  require(DIR_WS_LANGUAGES . $language . '/' . FILENAME_ADMIN_TOP_NOTES);
  
  function tep_set_admin_query_status($contr_id, $status) {
    if ($status == '0') {
      return tep_db_query("update " . TABLE_ADMIN_TOP_NOTES . " set status = '0', date_status_change = NULL where contr_id = '" . $contr_id . "'");
    } elseif ($status == '1') {
      return tep_db_query("update " . TABLE_ADMIN_TOP_NOTES . " set status = '1', date_status_change = now() where contr_id = '" . $contr_id . "'");
    } elseif ($status == '2') {
      return tep_db_query("update " . TABLE_ADMIN_TOP_NOTES . " set status = '2', date_status_change = now() where contr_id = '" . $contr_id . "'");
    } elseif ($status == '3') {
      return tep_db_query("update " . TABLE_ADMIN_TOP_NOTES . " set status = '3', date_status_change = now() where contr_id = '" . $contr_id . "'");
    } else {
      return -1;
    }
  }
  switch ($HTTP_GET_VARS['action']) {
    case 'setflag':
      ...
      break;
    case 'insert':
      if ($HTTP_POST_VARS['category_new'] != '') { tep_db_query("insert into " . TABLE_ADMIN_TOP_NOTES_TYPE . " (type_id, type_name, status) values ('1', '" . $HTTP_POST_VARS['category_new'] . "','1')"); }
      tep_db_query("insert into " . TABLE_ADMIN_TOP_NOTES . " (contr_id, category, admin_note, config_comments, note_created, status, last_update) values ('','" . $HTTP_POST_VARS['file_type_id'] . "','" . $HTTP_POST_VARS['admin_note_new'] . "','" . $HTTP_POST_VARS['config_comments'] . "', now(), '2', '" . $HTTP_POST_VARS['last_update'] . "' )");
      tep_redirect(tep_href_link(FILENAME_ADMIN_TOP_NOTES, '&sID=' . $contr_id));
      break;

    case 'copy_to':
      $product_query = tep_db_query("select p.contr_id, p.category, p.admin_note, p.config_comments, p.status, p.last_update from " . TABLE_ADMIN_TOP_NOTES . " p where p.contr_id = '" . $HTTP_GET_VARS['sID'] . "'");
      ...
      tep_db_query("insert into " . TABLE_ADMIN_TOP_NOTES . " (contr_id, category, admin_note, config_comments, note_created, status, last_update) values ('','" . $product['category'] . "','" . $corrected_admin_note . ' ' . TEXT_ADDED_COPY . "','" . $corrected_config_comments . "', now(), '3', '" . $product['last_update'] . "')");
      tep_redirect(tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sort=' . $HTTP_GET_VARS['sort'] . '&sID=' . $HTTP_GET_VARS['sID']));
      break;

    case 'update':
// Custom - Fix date_status_change
		  if ($HTTP_POST_VARS['date_status_change']) tep_db_query("update " . TABLE_ADMIN_TOP_NOTES . " set date_status_change = now() " . " where contr_id = '" . $HTTP_POST_VARS['contr_id'] . "'");
// Custom - Use $HTTP_POST_VARS with register_globals = off
 	  tep_db_query("update " . TABLE_ADMIN_TOP_NOTES . " set contr_last_modified = now(), status = '" . $HTTP_POST_VARS['status'] . "', admin_note = '" . $HTTP_POST_VARS['admin_note_new'] . "', category =  '" . $HTTP_POST_VARS['file_type_id'] . "', config_comments = '" . $HTTP_POST_VARS['config_comments'] . "', last_update = '" . $HTTP_POST_VARS['last_update'] . "' where contr_id = '" . $HTTP_POST_VARS['contr_id'] . "'");
	  tep_redirect(tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_POST_VARS['page'] . '&sID=' . $HTTP_POST_VARS['contr_id'] . '&sort=' . $HTTP_POST_VARS['sort']));
      break;

    case 'deleteconfirm':
      $contr_id = tep_db_prepare_input($HTTP_GET_VARS['sID']);
      tep_db_query("delete from " . TABLE_ADMIN_TOP_NOTES . " where contr_id = '" . tep_db_input($contr_id) . "'");
      tep_redirect(tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sort=' . $HTTP_GET_VARS['sort']));
      break;
  }
?>
...
if ($HTTP_GET_VARS['action'] != 'new' || $HTTP_GET_VARS['action'] != 'edit') {
    ...
    $file_type_query = tep_db_query("select type_name from " . TABLE_ADMIN_TOP_NOTES_TYPE . " order by type_name");
...
?>
            <td align="right"><table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr><?php echo tep_draw_form('search', FILENAME_ADMIN_TOP_NOTES, '', 'get'); ?>
              ...
              <tr><?php echo tep_draw_form('search_type', FILENAME_ADMIN_TOP_NOTES, '', 'get'); ?>
...
<?php
  if ( ($HTTP_GET_VARS['action'] == 'new') || ($HTTP_GET_VARS['action'] == 'edit') ) {
    ...
    if ( ($HTTP_GET_VARS['action'] == 'edit') && ($HTTP_GET_VARS['sID']) ) {
      ...
      $product_query = tep_db_query("select p.contr_id, p.category, p.admin_note, p.status, p.config_comments, p.last_update from " . TABLE_ADMIN_TOP_NOTES . " p where p.contr_id = '" . $HTTP_GET_VARS['sID'] . "'");
      $product = tep_db_fetch_array($product_query);
...
<script language="javascript">
 ...
 ?>
      <tr><form name="new_contr" <?php echo 'action="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, tep_get_all_get_params(array('action', 'info', 'sID', 'date_status_change')) . 'action=' . $form_action, 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo tep_draw_hidden_field('contr_id', $HTTP_GET_VARS['sID']); if($initial_status != $HTTP_GET_VARS['status']) echo tep_draw_hidden_field('date_status_change', 'true') ?>
...
<?php } ?>          
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main" align="right" valign="top"><br><?php echo (($form_action == 'insert') ? tep_image_submit('button_insert.png', IMAGE_INSERT) : tep_image_submit('button_update.png', IMAGE_UPDATE)). '&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $HTTP_GET_VARS['sID']) . '&sort=' . $HTTP_GET_VARS['sort'] . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'sID=' . $HTTP_GET_VARS['sID']) . '&action=' . $HTTP_GET_VARS['action'] . '" onclick="reset();">' . tep_image_button('button_reset.png', IMAGE_RESET) . '</a>'; ?></td>
          </tr>
        </table></td>
      </form>
      </tr>
...
<?php
    ...
    $admin_query_query_raw = "select contr_id, admin_note, category, status, note_created , contr_last_modified , date_status_change , last_update from " . TABLE_ADMIN_TOP_NOTES . " where admin_note like '%" . $HTTP_GET_VARS['search'] . "%' or category like '%" . $HTTP_GET_VARS['search'] . "%' order by " . $order_it_by . " ";
    ...
    $admin_query_query_raw = "select contr_id, admin_note, category, status, note_created , contr_last_modified , date_status_change , last_update from " . TABLE_ADMIN_TOP_NOTES . " where category = '" . $HTTP_GET_VARS['file_type_id'] . "' order by " . $order_it_by . " ";
    $admin_query_split = new splitPageResults($HTTP_GET_VARS['page'], MAX_DISPLAY_SEARCH_RESULTS, $admin_query_query_raw, $admin_query_query_numrows);
    $admin_query_query = tep_db_query($admin_query_query_raw);
    if (tep_db_num_rows($admin_query_query) == 0) echo '<td  class="dataTableContent">' . TEXT_EMPTY_CATEGORY . '</td>';
    } else {            
    $admin_query_query_raw = "select contr_id, admin_note, category, status, note_created , contr_last_modified , date_status_change , last_update from " . TABLE_ADMIN_TOP_NOTES . " order by " . $order_it_by . " ";
    $admin_query_split = new splitPageResults($HTTP_GET_VARS['page'], MAX_DISPLAY_SEARCH_RESULTS, $admin_query_query_raw, $admin_query_query_numrows);
    $admin_query_query = tep_db_query($admin_query_query_raw);
...
if ($HTTP_GET_VARS['search'] == '') {
      if ( (is_object($sInfo)) && ($admin_quer['contr_id'] == $sInfo->contr_id)) {
        echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id . '&sort=' . $HTTP_GET_VARS['sort'] . '&action=readonly') . '\'">' . "\n";
      } else {
        echo '                  <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id']) . '&sort=' . $HTTP_GET_VARS['sort'] . '\'">' . "\n"; }
} else {
    $searchresult_query_query_raw = "select contr_id from " . TABLE_ADMIN_TOP_NOTES . " order by " . $order_it_by . " ";
    ...
    $searchresult_page= round($searchresult_query_query_numrows / MAX_DISPLAY_SEARCH_RESULTS)-1;
      ...
      if ( (is_object($sInfo)) && ($admin_quer['contr_id'] == $sInfo->contr_id)) {
        ...
        echo '                  <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $searchresult_page . '&sID=' . $admin_quer['contr_id']) . '&sort=' . $HTTP_GET_VARS['sort'] . '\'">' . "\n"; }
}
?>
...
<?php
      if ($admin_quer['status'] == '0') {
                echo tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_red.gif', IMAGE_NOTES_ICON_STATUS_RED, 10, 10) . '&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=setflag&flag=1&id=' . $admin_quer['contr_id'] . '&page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_green_light.gif', IMAGE_NOTES_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=setflag&flag=2&id=' . $admin_quer['contr_id'] . '&page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_yellow_light.gif', IMAGE_NOTES_ICON_STATUS_YELLOW_LIGHT, 10, 10) . '</a>';
      } elseif ($admin_quer['status'] == '1') {
                       echo '<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=setflag&flag=0&id=' . $admin_quer['contr_id'] . '&page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_red_light.gif', IMAGE_NOTES_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>&nbsp;' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_green.gif', IMAGE_NOTES_ICON_STATUS_GREEN, 10, 10) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=setflag&flag=2&id=' . $admin_quer['contr_id'] . '&page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_yellow_light.gif', IMAGE_NOTES_ICON_STATUS_YELLOW_LIGHT, 10, 10) . '</a>';
      } elseif ($admin_quer['status'] == '2') {
                       echo '<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=setflag&flag=0&id=' . $admin_quer['contr_id'] . '&page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_red_light.gif', IMAGE_NOTES_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=setflag&flag=1&id=' . $admin_quer['contr_id'] . '&page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_green_light.gif', IMAGE_NOTES_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=setflag&flag=3&id=' . $admin_quer['contr_id'] . '&page=' . $HTTP_GET_VARS['page'] . '&sID=' . $admin_quer['contr_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_yellow.gif', IMAGE_NOTES_ICON_STATUS_YELLOW, 10, 10) . '</a>';
      } else {
...
?>
                  <tr> 
                    <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'action=new') . '">' . tep_image_button('button_insert.png', IMAGE_INSERT) . '</a>'; ?></td>
                  </tr>
<?php
  } 
?>  
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {
    ...
    case 'delete':
      ...
      $contents = array('form' => tep_draw_form('install_contr_del', FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id . '&sort=' . $HTTP_GET_VARS['sort'] . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_INFO_DELETE_INTRO);
      $contents[] = array('text' => '<br>' . TABLE_HEADING_NAME . ': <b>' . $sInfo->admin_note . '</b>');
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id) . '&sort=' . $HTTP_GET_VARS['sort'] . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (is_object($sInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id . '&sort=' . $HTTP_GET_VARS['sort'] . '&action=edit') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id . '&sort=' . $HTTP_GET_VARS['sort'] . '&action=delete') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a><br><br> <a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id . '&sort=' . $HTTP_GET_VARS['sort'] . '&action=readonly') . '">' . tep_image_button('button_preview.png', IMAGE_ICON_VIEW) . '</a> <a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id . '&sort=' . $HTTP_GET_VARS['sort'] . '&action=copy_to') . '">' . tep_image_button('button_copy.png', IMAGE_COPY) . '</a>' );
      }
      break;
  }
...
// BOF readonly
      $product_query = tep_db_query("select contr_id, category, admin_note, status, config_comments, date_status_change, note_created, contr_last_modified, last_update from " . TABLE_ADMIN_TOP_NOTES . " where contr_id = '" . $HTTP_GET_VARS['sID'] . "'");
      $product = tep_db_fetch_array($product_query);
      $sInfo = new objectInfo($product);
...
?>
      ...
      <tr>
            <td class="main"><?php echo '<a href="' . tep_href_link(FILENAME_ADMIN_TOP_NOTES, 'page=' . $HTTP_GET_VARS['page'] . '&sID=' . $sInfo->contr_id . '&sort=' . $HTTP_GET_VARS['sort'], 'NONSSL') . '">' . tep_image_button('button_back.png', IMAGE_BUTTON_BACK) . '</a>'; ?></td>
      </tr>      
<?php          
} // EOF readonly         
?>
