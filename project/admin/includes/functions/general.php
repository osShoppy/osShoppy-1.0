<?php //$Id: /catalog/admin/includes/functions/general.php (osC)

// FUNCTION MOVED FROM password_funcs.php and password_funcs_create_account.php \\
////
...
////
// BOF-A: Administration Acces Level Accounts /Step-1(2)\
...
// BOF-A: Administration Acces Level Accounts /Step-2(2)\
...
// EOF-A: Administration Acces Level Accounts /Step-2(2)\
...
//function tep_info_image($image, $alt, $width = '', $height = '') {
  function tep_info_image($image, $alt = '', $width = '', $height = '') { // 2gether discount
...
function tep_get_products_spec($product_id, $language_id) {
    ...
    return $product['products_spec'];
  }
  function tep_get_products_musthave($product_id, $language_id) {
    $product_query = tep_db_query("select products_musthave from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_musthave'];
  }
  function tep_get_products_extraimage($product_id, $language_id) {
    $product_query = tep_db_query("select products_extraimage from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_extraimage'];
  }
  function tep_get_products_manual($product_id, $language_id) {
    $product_query = tep_db_query("select products_manual from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_manual'];
  }
  function tep_get_products_extra1($product_id, $language_id) {
    $product_query = tep_db_query("select products_extra1 from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_extra1'];
  }
  function tep_get_products_moreinfo($product_id, $language_id) {
    $product_query = tep_db_query("select products_moreinfo from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . (int)$product_id . "' and language_id = '" . (int)$language_id . "'");
    $product = tep_db_fetch_array($product_query);

    return $product['products_moreinfo'];
  }
...
// beof - additional images added:  "products_image_med,products_image_pop"
    $product_image_query = tep_db_query("select products_image, products_image_med, products_image_pop from " . TABLE_PRODUCTS . " where products_id = '" . (int)$product_id . "'");
...
// bof -  additional images 
      if (!empty($product_image['products_image_med']) && file_exists(DIR_FS_CATALOG_IMAGES . $product_image['products_image_med'])) {
        @unlink(DIR_FS_CATALOG_IMAGES . $product_image['products_image_med']);
      }
      if (!empty($product_image['products_image_pop']) && file_exists(DIR_FS_CATALOG_IMAGES . $product_image['products_image_pop'])) {
        @unlink(DIR_FS_CATALOG_IMAGES . $product_image['products_image_pop']);
      }
	  // look for original image name from medium image name
	  if (!empty($product_image['products_image_med']) && file_exists(DIR_FS_CATALOG_IMAGES.substr($product_image['products_image_med'],0,-5).substr($product_image['products_image_med'],-4,4))) {
	    @unlink(DIR_FS_CATALOG_IMAGES.substr($product_image['products_image_med'],0,-5).substr($product_image['products_image_med'],-4,4));
	  }
