<?php //$Id: /catalog/admin/includes/functions/links.php (5272) 
...
function CheckURL($url, $links_id) {
    ...
    if (tep_not_null($lines)) {
        ...
        $link_check_status_text = (($found == true) ? TEXT_INFO_LINK_CHECK_FOUND : TEXT_INFO_LINK_CHECK_NOT_FOUND);
    } else  {
        $link_check_status_text = TEXT_INFO_LINK_CHECK_ERROR;
    }

    $links_check_query = tep_db_query("SELECT date_last_checked, link_found FROM " .  TABLE_LINKS_CHECK . " where links_id = " . (int)$links_id . " limit 1");
...
/***************************************************************************
Checks to see if the various domain names match. The function will either
return a string with yes or no or a bool value. The login is reversed on
these so that a yes for a string would equal a false for bool. This is to
make it easier for the calling code to handle the results.
***************************************************************************/
function DomainMismatch($lInfo, $rtnBool = false) {
    if ($lInfo->links_reciprocal_disable) {
        return ($rtnBool ? false : TEXT_DOMAIN_MISMATCH_DISABLED);
    }

    $domainMismatch = ($rtnBool ? true : TEXT_DOMAIN_MISMATCH_NO);
    $mainDomain = GetDomainName($lInfo->links_url);
    $imageDomain = ((strpos($lInfo->links_image_url, 'http') === FALSE) ? '' : GetDomainName($lInfo->links_image_url));

    if (GetDomainName($lInfo->links_reciprocal_url) !==  $mainDomain || (! empty($imageDomain) && $imageDomain !==  $mainDomain)) {
        $domainMismatch = ($rtnBool ? false : TEXT_DOMAIN_MISMATCH_YES);
    }
...
/***************************************************************************
    return an array of the recipricol pages content
***************************************************************************/
function GetLinksFileArray($url)
{
  ...
  if (function_exists('curl_init')) { //use curl if possible to read in site information
      ...
      if(curl_errno($ch))
          return $lines; //echo 'Curl error: ' . curl_error($ch);
...
function GetLinksLanguagesArray() //build special language array for Links Manager
{
...
function GetHeaderStatus($url) {
    ...
    switch ($headerStatus[0]) {
       case 2: $siteFound = TEXT_INFO_LINK_LOCATED_YES; break;
       case 3: $siteFound = TEXT_INFO_LINK_LOCATED_YES_REDIRECTED; break;
       case 4:
       case 5:
       default: $siteFound = TEXT_INFO_LINK_LOCATED_NO;
    }
...
function NoParentLink($lInfo)
{
    ...
    return TEXT_DOMAIN_MISMATCH_YES;
}
...
function tep_link_info_image($image, $alt, $width = '', $height = '') {
  if (tep_not_null($image)) {
      $image = ((strpos($src, 'http') === FALSE) ? HTTP_SERVER . DIR_WS_CATALOG_IMAGES . 'uploads_links_manager/' . $image : $image);
