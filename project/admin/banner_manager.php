<?php //$Id: /catalog/admin/banner_manager.php (osC)
...
// os6tm banner manager
    if (isset($HTTP_GET_VARS['cid']) && $HTTP_GET_VARS['cid'] != '') {
    	if (isset($HTTP_GET_VARS['gestban']) && $HTTP_GET_VARS['gestban'] != '') {
    		...
    		switch($gestban) {
    			case 'add':
    			$category_query = tep_db_query("select * from " . TABLE_BANNERS_TO_CATEGORIES . " where banners_id = '" . $bID . "' and categories_id = '" . $cid ."'");
    			if (tep_db_num_rows($category_query) == 0) {
    				$category_query = tep_db_query("insert into " . TABLE_BANNERS_TO_CATEGORIES . " VALUES ('" . $bID . "','" . $cid ."')");
    				}
    			break;
    			case 'remove':
    			$category_query = tep_db_query("delete from " . TABLE_BANNERS_TO_CATEGORIES . " where categories_id = '" . $cid . "'");
    			break;
    			case 'addall':
    			$category_query = tep_db_query("select * from " . TABLE_BANNERS_TO_CATEGORIES . " where banners_id = '" . $bID . "' and categories_id = '" . $cid ."'");
    			if (tep_db_num_rows($category_query) == 0) {
    			...
    			if (tep_has_category_subcategories($cid)) {
    				$subcategories = array();
    				tep_get_subcategories($subcategories, $cid);
    				while(list(,$category) = each($subcategories)) {
    					$category_query = tep_db_query("select * from " . TABLE_BANNERS_TO_CATEGORIES . " where banners_id = '" . $bID . "' and categories_id = '" . $category ."'");
		    			if (tep_db_num_rows($category_query) == 0) {
		    				$category_query = tep_db_query("insert into " . TABLE_BANNERS_TO_CATEGORIES . " VALUES (" . $bID . "," . $category .")");
		    				}
    					}
    				}
    			break;
    			...
    			if (tep_has_category_subcategories($cid)) {
    				...
    				tep_get_subcategories($subcategories, $cid);
    				while(list(,$category) = each($subcategories)) {
    					$category_query = tep_db_query("delete from " . TABLE_BANNERS_TO_CATEGORIES . " where categories_id = '" . $category . "'");
    					}
    				}
    			break;
    			}
  ...
  var dateScheduled = new ctlSpiffyCalendarBox("dateScheduled", "new_banner", "date_scheduled","btnDate2","<?php echo $bInfo->date_scheduled; ?>",scBTNMODE_CUSTOMBLUE);
      ...
      <tr><?php echo tep_draw_form('new_banner', FILENAME_BANNER_MANAGER, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo tep_draw_hidden_field('banners_id', $bID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
          <td valign="top" class="main"><?php echo TEXT_BANNER_MANAGER_INDEX?></td>
          <td class="main">
          <?php
// os6tm banner manager
          ...
          <tr>
          <td valign="top" class="main"><?php echo TEXT_BANNER_MANAGER?></td>
          <td class="main">
          <script language="javascript">
          <!--
          function select_all() {
          ...
          while (list(,$category) = each($categories)) {
          	...
          	if ($category['parent_id'] == 0) {
          		...
          		if (($i % 3) > 0) {
          			$categories_string = substr($categories_string, 1, strlen($categories_string));
          			echo tep_draw_separator();
          			echo '<font size="1">�</font> <a href="javascript:onclick=select_all(' . $categories_string . ')">' . TEXT_BANNERS_SELECT_ALL . '</a>';
          			echo '<br><font size="1">�</font> <a href=javascript:onclick=unselect_all(' . $categories_string . ')>' . TEXT_BANNERS_UNSELECT_ALL . '</a>';
          	...
          	echo tep_draw_checkbox_field('categories[' . $j . ']', $category['id'], $is_selected) . '<a href="' . tep_href_link(FILENAME_CATEGORIES, tep_get_path($category['parent_id']) . '&cID=' . $category['id']) . '">' . $category['text'] . '</a>';
          	echo '<br>';
          	$categories_string .= ',' . $j;
          	$j++;
	}
	...
	$categories_string = substr($categories_string, 1, strlen($categories_string));
	echo tep_draw_separator();
	echo '� <a href=javascript:onclick=select_all(' . $categories_string . ')>' . TEXT_BANNERS_SELECT_ALL . '</a>';
	echo '<br><font size="1">�</font> <a href=javascript:onclick=unselect_all(' . $categories_string . ')>' . TEXT_BANNERS_UNSELECT_ALL . '</a>';
	while (($i % 3) > 0) {
		if (($i % 3) > 2) {
			echo '</td><td>';
...
// os6tm banner manager_eof
      ...
      <tr>
            <td class="main" align="right" valign="top" nowrap><?php echo (($form_action == 'insert') ? tep_image_submit('button_insert.png', IMAGE_INSERT) : tep_image_submit('button_update.png', IMAGE_UPDATE)). '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . (isset($HTTP_GET_VARS['bID']) ? 'bID=' . $HTTP_GET_VARS['bID'] : '')) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>'; ?></td>
      </tr>
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main"><?php echo TEXT_BANNERS_BANNER_GROUPS ?></td>
          </tr>
          <tr>
            <td class="main" width="50%" valign="top"><?php echo TEXT_BANNER_GROUPS ?></td>
            <td class="main" width="50%" valign="top"><?php echo TEXT_BANNER_GROUPS_PAGE ?></td>
          </tr>
          <tr>
...
?>
                <td class="dataTableContent"><?php echo '<a href="javascript:popupImageWindow(\'' . FILENAME_POPUP_IMAGE . '?banner=' . $banners['banners_id'] . '\')">' . tep_image(DIR_WS_ICONS . 'banner_icons/icon_popup.gif', 'View Banner') . '</a>&nbsp;' . $banners['banners_title']; ?></td>
...
<?php
      if ($banners['status'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_green.gif', 'Active', 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=0') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_red_light.gif', 'Set Inactive', 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=1') . '">' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_green_light.gif', 'Set Active', 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icons/status_icons/icon_status_red.gif', 'Inactive', 10, 10);
      }
?></td>
                <td class="dataTableContent" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_BANNER_STATISTICS, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $banners['banners_id']) . '">' . tep_image(DIR_WS_ICONS . 'banner_icons/icon_statistics.gif', ICON_STATISTICS) . '</a>&nbsp;'; if (isset($bInfo) && is_object($bInfo) && ($banners['banners_id'] == $bInfo->banners_id)) { echo tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $banners['banners_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $HTTP_GET_VARS['bID']) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (is_object($bInfo)) {
        $heading[] = array('text' => '<b>' . $bInfo->banners_title . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $bInfo->banners_id . '&action=new') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $HTTP_GET_VARS['page'] . '&bID=' . $bInfo->banners_id . '&action=delete') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>');
