<?php //$Id: /catalog/admin/admin_account.php (1359)
    ...
    switch ($HTTP_GET_VARS['action']) {
      ...
      case 'save_account':
        ...
        if (in_array($HTTP_POST_VARS['admin_email_address'], $stored_email)) {
		  ...
		  tep_mail(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, sprintf(ADMIN_EMAIL_SUBJECT, STORE_NAME, $HTTP_POST_VARS['admin_firstname'], $HTTP_POST_VARS['admin_lastname']), sprintf(ADMIN_EMAIL_TEXT, $HTTP_POST_VARS['admin_firstname'], HTTP_SERVER . DIR_WS_ADMIN, $HTTP_POST_VARS['admin_email_address'], tep_db_prepare_input($HTTP_POST_VARS['admin_password']), STORE_OWNER), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
...
<!-- body_text //-->
    <td width="100%" valign="top">
      ...
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0" class="contentHeading">
          <tr>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0" align="center" class="content">
          <tr>
            <td valign="top">
<?php
...
?>
              ...
              <tr>
                <td><table width="100%" border="0" cellspacing="0" cellpadding="3"><tr><td class="smallText" valign="top"><?php echo TEXT_INFO_MODIFIED . $myAccount['admin_modified']; ?></td><td align="right"><?php if ($HTTP_GET_VARS['action'] == 'edit_process') { echo '<a href="' . tep_href_link(FILENAME_ADMIN_ACCOUNT) . '">' . tep_image_button('button_back.png', IMAGE_BACK) . '</a> '; if (tep_session_is_registered('confirm_account')) { echo tep_image_submit('button_save.png', IMAGE_SAVE, 'onClick="validateForm();return document.returnValue"'); } } elseif ($HTTP_GET_VARS['action'] == 'check_account') { echo '&nbsp;'; } else { echo tep_image_submit('button_edit.png', IMAGE_EDIT); } ?></td><tr></table></td>
              </tr>
            </table>
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {
    ...
    case 'check_account':
      ...
      $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ADMIN_ACCOUNT) . '">' . tep_image_button('button_back.png', IMAGE_BACK) . '</a> ' . tep_image_submit('button_confirm.png', IMAGE_CONFIRM) . '<br>&nbsp');
      break;
    default:
      $heading[] = array('text' => '<b>&nbsp;' . TEXT_INFO_HEADING_DEFAULT . '</b>');
      ...
      if ($myAccount['admin_email_address'] == 'change@mail.nl') {
