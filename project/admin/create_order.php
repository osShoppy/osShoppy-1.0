<?php //$Id: /catalog/admin/create_order.php (4829)

  require('includes/application_top.php');

  // #### Get Available Customers

  $query = tep_db_query("select a.customers_id, a.customers_firstname, a.customers_insertion, a.customers_lastname, b.entry_company, b.entry_city, c.zone_code from " . TABLE_CUSTOMERS . " AS a, " . TABLE_ADDRESS_BOOK . " AS b LEFT JOIN " . TABLE_ZONES . " as c ON (b.entry_zone_id = c.zone_id) WHERE a.customers_default_address_id = b.address_book_id  ORDER BY entry_company,customers_lastname");
  $result = $query;
  ...
  if ($customer_count > 0){
    ...
    while($db_Row = tep_db_fetch_array($result)){ 
      ...
      if(isSet($HTTP_GET_VARS['Customer']) and $db_Row['customers_id']==$HTTP_GET_VARS['Customer']){
        ...
        $SelectCustomerBox .= ">" . (empty($db_Row['entry_company']) ? "": strtoupper($db_Row['entry_company']) . " - " ) . $db_Row['customers_lastname'] . ".&nbsp;" . $db_Row['customers_insertion'] . ", " . $db_Row['customers_firstname'] . " - " . $db_Row['entry_city'] . ", " . $db_Row['zone_code'] . "</option>\n";
      }else{
        $SelectCustomerBox .= ">" . (empty($db_Row['entry_company']) ? "": strtoupper($db_Row['entry_company']) . " - " ) . $db_Row['customers_lastname'] . ".&nbsp;" . $db_Row['customers_insertion'] . " , " . $db_Row['customers_firstname'] . " - " . $db_Row['entry_city'] . ", " . $db_Row['zone_code'] . "</option>\n";
      }
    }
...
<!-- header_eof //-->		
  ...
  <td valign="top">
    ...
    <table border="0" width="90%" class="dataTableHeadingRow" align="center">
      <tr>
    ...
    <tr>
      <table border="0" width="90%" cellspacing="0" cellpadding="2">
        <tr>
          <td class="main" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_DEFAULT, '', 'SSL') . '">' . tep_image_button('button_back.png', IMAGE_BACK) . '</a>'; ?>&nbsp;<?php echo tep_image_submit('button_continue.png', IMAGE_SAVE); ?></td>
        </tr>
      </table></form>
    </tr>
    <tr>
