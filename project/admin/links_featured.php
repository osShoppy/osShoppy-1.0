<?php //$Id: /catalog/admin/link_featured.php (5272)
    ...
    else
    {
      ...
      if ($_POST['day'] && $_POST['month'] && $_POST['year']) 
      {
         ...
         {         
            ...
            $existingLink_query = tep_db_query("select count(*) as total from " . TABLE_LINKS_FEATURED . " where links_id = '" . (int)$_POST['link_partners'] . "'");
            $existingLink = tep_db_fetch_array($existingLink_query);

            $showAllPages = ($_POST['links_all_pages'] == 'on') ? 1 : 0;
            
            if ($existingLink['total'] > 0) {
              tep_db_query("update " . TABLE_LINKS_FEATURED . " SET date_added = now(), expires_date = '" . tep_db_input($expires_date) . "', links_all_pages = '" . $showAllPages . "' where links_id = '" . (int)$_POST['link_partners'] . "'");
            } else {
              tep_db_query("insert into " . TABLE_LINKS_FEATURED . " (links_id, date_added, expires_date, links_all_pages) values ('" . $_POST['link_partners'] . "', now(), '" . tep_db_input($expires_date) . "', '" . $showAllPages . "')");
            }
         }
         else
         {
...
<!-- body_text //-->
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="85%" cellspacing="0" cellpadding="2" class="linksBordered">
          <tr>
            <td class="linksHead"><?php echo HEADING_TITLE_LINKS_FEATURED; ?></td>
