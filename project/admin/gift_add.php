<?php //$Id: /catalog/admin/gift_add.php (2990)
    ...
    switch ($action) {
      case 'addGift':
       ...
       if (tep_not_null($action_gift))	{
	      $product_query = tep_db_query("SELECT products_id, language_id, products_name FROM products_description WHERE language_id = '".$languages_id."' AND products_id = '" . $action_gift ."'");
		...
		if ($id && $threshold) {
	  		$SQL = tep_db_query("INSERT INTO " . TABLE_CARROT . " (threshold, products_id) VALUES ('".$threshold."','".$id."')");
		} else {
	  ...
	  case 'delete' :
	  	$id = $HTTP_GET_VARS['ID'];
		if ($id) {
		    tep_db_query("DELETE FROM " . TABLE_CARROT . " WHERE products_id = '".$id."'");
		}
		tep_redirect('gift_add.php');
        break;
    }
  ...
  while ($gift_list = tep_db_fetch_array($gift_list_query))
   $freeGifts[] = array('id' => $gift_list['products_id'], 'text' => $gift_list['products_name']);
...
ORDER BY fg.threshold ASC");
			...
			while ($gift = tep_db_fetch_array($gift_query)) {
				...
				<td class="smallText">&euro;<input type="text" size="4" name="threshold" value="'.$gift['threshold'].'"></td>
				<td class="smallText">'.$gift['products_id'].'</td>
				<td class="smallText">'.$gift['products_name'].'</td>
			<td><a href="gift_add.php?ID=' . $gift['products_id'] . '&action=delete">' . tep_image_button('button_delete.png', 'Delete') . '</a>&nbsp;<a href="categories.php?pID=' . $gift['products_id'] . '&action=new_product">' . tep_image_button('button_edit.png', 'Edit') . '</a></td>
				</tr></form>';
			}
		?>
     </table></td>
    ...
    <tr>
     <td><table border="0">
      ...
      <form name="newGift" method="GET">
	    <tr>
	     <td class="smallText">&euro; <input type="text" name="threshold" size="4"><?php echo ' ' . TEXT_THRESHOLD; ?></td>
       <td witdh="40">&nbsp;</td>
