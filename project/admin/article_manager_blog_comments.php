<?php //$Id: /catalog/admin/article_manager_blog_comments.php (1709)
  ...
  if (tep_not_null($action)) {
    ...
    switch ($action) {
      ...
      case 'save':
        ...
        tep_redirect(tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'comID=' . $articles_id));
        break;
      case 'deleteconfirm':
        ...
        tep_redirect(tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page']));
        break;
    }
  }
...
?>
      ...
      <tr><?php echo tep_draw_form('authors', FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'action=insert', 'post', 'enctype="multipart/form-data"'); ?>
...
?>
          <tr>
            <td colspan="3" class="main" align="center"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_image_submit('button_save.png', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&auID=' . $_GET['auID']) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>'; ?></td>
          </tr>
        </table></td>
      </form>
      </tr>
...
?>
      ...
      <tr><?php echo tep_draw_form('comments', FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&comID=' . $comments['articles_id'] . '&action=save', 'post', 'enctype="multipart/form-data"'); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <table border="0" cellspacing="0" cellpadding="0">
              <tr>
          ...
          <tr>
            <td colspan="3" class="main" align="center"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_image_submit('button_save.png', IMAGE_SAVE) . ' <a href="' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&auID=' . $authors['authors_id']) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>'; ?></td>
          </tr>
        </table></td>
      </form>
      </tr>
...
?>
      <tr>
        <td class="main" colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&auID=' . $authors['authors_id']) . '">' . tep_image_button('button_back.png', IMAGE_BACK) . '</a>'; ?></td>
      </form>
      </tr>
          </tr>
        </table></td>
...
<?php
  ...
  while ($comments = tep_db_fetch_array($comments_query)) {
      ...
      if (isset($comInfo) && is_object($comInfo) && ($comments['articles_id'] == $comInfo->articles_id)) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&comID=' . $comments['articles__id'] . '&action=edit') . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&comID=' . $comments['articles_id']) . '\'">' . "\n";
      }
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($comInfo) && is_object($comInfo) && ($comments['articles_id'] == $comInfo->articles_id)) { echo tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&auID=' . $comments['articles_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
  }
?>
...
<?php
  ...
  switch ($action) {
    case 'delete':
      ...
      $contents = array('form' => tep_draw_form('articles', FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&comID=' . $comInfo->articles_id . '&action=deleteconfirm'));
      $contents[] = array('text' => TEXT_DELETE_INTRO);
      $contents[] = array('text' => '<br><b>' . $comnfo->articles_name . '</b>');

      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.png', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&comID=' . $comInfo->articles_id) . '">' . tep_image_button('button_cancel.png', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (isset($comInfo) && is_object($comInfo)) {
        $heading[] = array('text' => '<b>' . $comInfo->articles_name . '</b>');

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&comID=' . $comInfo->articles_id . '&action=edit') . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ARTICLE_MANAGER_BLOG_COMMENTS, 'page=' . $_GET['page'] . '&comID=' . $comInfo->articles_id . '&action=delete') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>');
