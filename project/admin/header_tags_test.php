<?php //$Id: /catalog/admin/header_tags_test.php (5581)
  ...
  if (isset($_POST['action']) && $_POST['action'] == 'test')
  {
    ...
    for ($i = 0; $i < count($files); ++$i)
    {
      ...
      else if (file_exists($files[$i]) && strpos($files[$i], "install.php") !== FALSE)
      {
         $extraFiles[] = $files[$i];
         $found = true;
      }  
    ...
    $dbError = false;
    $files = ($stsInstalled) ? $hdrFiles : GetBaseFiles();
...
//    else if ($notFirst && $i == 0)  //show what is there
    ...
    if (isset($_POST['check_url_page']) && substr($_POST['check_url_page'], -4) == '.php')
    {
       ...
       for ($i = 0; $i < count($languages); ++$i)
       {
           ...
           $results[] = sprintf(ERROR_META_DATA_TITLE, (tep_not_null($metaData['title'][1]) ? 'yellow' : 'red'), $metaData['title'][1]);
           $results[] = sprintf(ERROR_META_DATA_DESC, (tep_not_null($metaData['description'][1]) ? 'yellow' : 'red'), $metaData['description'][1]);
           $results[] = sprintf(ERROR_META_DATA_KEYWORDS, (tep_not_null($metaData['keywords'][1]) ? 'yellow' : 'red'), $metaData['keywords'][1]);
           $results[] = ERROR_META_DATA_TEXT_SHOULD_BE;
           
           $pageInfo_query = tep_db_query("select page_title as title, page_description as description, page_keywords as keywords, append_category, append_product, append_default_title, append_default_description, append_default_keywords, sortorder_title, sortorder_description, sortorder_keywords, sortorder_root, sortorder_product, sortorder_category from " . TABLE_HEADERTAGS . " where page_name = '" . $_POST['check_url_page'] . "' and language_id = " . (int)$languages[$i]['id'] . " limit 1");
