<?php //$Id: /catalog/admin/orders.php (osC)
...
if (EMAIL_USE_HTML == 'true') {
	require(DIR_FS_CATALOG_MODULES . 'html_mail/'. HTML_EMAIL_LAYOUT .'/orders.php');
	$email = $html_email;
...
?>
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo HEADING_TITLE . (($order->customer['is_dummy_account'])? '</td><td class="main"><b>' . TEXT_NO_ACCOUNT . '</b>':''); ?></td>
            <td class="pageHeading" align="right"><?php echo tep_draw_separator('pixel_trans.gif', 1, HEADING_IMAGE_HEIGHT); ?></td>
			<td class="pageHeading" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS_EDIT, 'oID=' . $_GET['oID']) . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_invoice.png', IMAGE_ORDERS_INVOICE) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_packingslip.png', IMAGE_ORDERS_PACKINGSLIP) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action'))) . '">' . tep_image_button('button_back.png', IMAGE_BACK) . '</a> '; ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<!-- // BOF-A Separate Pricing Per Customer ---//Step-1(2)\\--->
              <tr>
...
<!-- // EOF-A Separate Pricing Per Customer ---//Step-1(2)\\--->
            </table></td>
            <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="2">
              <tr>
      ...
      <tr>
        <td><hr style="color:#000000" size="1"><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main"><b><?php echo ENTRY_ORDER_ID; ?></b></td>
            <td class="main"><?php echo tep_db_input($oID); ?></td>
          </tr>
          <tr>
            <td class="main"><b><?php echo ENTRY_DATE_TIME; ?></b></td>
      ...
      <tr>
        <td><hr style="color:#000000" size="1"><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main"><b><?php echo TEXT_INFO_CUSTOMER_NOTES_BACKEND; ?></b></td>
<?php 
...
// BOF-A: Admin Notes ---//Step-1(1)\\---
?>
            <td class="main"><?php echo $cust_notes['customers_notes'];?></td>
          </tr>
          <tr>
            <td class="main"><hr style="color:#000000" size="1"><b><?php echo TEXT_INFO_CUSTOMER_NOTES_FRONTEND; ?></b></td>
<?php 
// BOF-A: Admin Notes ---//Step-1(1)\\---
$cust_notes_query2 = tep_DB_query("select customers_id from " . TABLE_ORDERS . " where orders_id = '" . (int)$oID . "'");
$cust_notes2 = tep_db_fetch_array($cust_notes_query2);
$cust_notes_query = tep_db_query("select customers_id, customers_notes_frontend from " . TABLE_CUSTOMERS . " where customers_id = '" . $cust_notes2['customers_id'] . "'");
$cust_notes = tep_db_fetch_array($cust_notes_query);
// BOF-A: Admin Notes ---//Step-1(1)\\---
?>
            <td class="main"><?php echo $cust_notes['customers_notes_frontend'];?></td>
          </tr>
        </table><hr style="color:#000000" size="1"></td>
      </tr>
      <tr>
...
<!-- // bof - points and rewards check_box_bof //-->
<?php
    if ((USE_POINTS_SYSTEM == 'true') && !tep_not_null(POINTS_AUTO_ON)) {
...
?>
<!-- // eof - points and rewards check_box_bof //-->
          </tr>
        </table></td>
      </form></tr>
      <tr>
	<td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS_EDIT, 'oID=' . $_GET['oID']) . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS_INVOICE, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_invoice.png', IMAGE_ORDERS_INVOICE) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_packingslip.png', IMAGE_ORDERS_PACKINGSLIP) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action'))) . '">' . tep_image_button('button_back.png', IMAGE_BACK) . '</a> '; ?></td>      </tr>
<?php
  } else {
?>
      <tr>
...
// BOF-A Separate Pricing Per Customer ---//Step-2(2)\\---
	...
	// manual_order_maker added o.customer_service_id
...
// EOF-A Separate Pricing Per Customer ---//Step-2(2)\\---
    }
...
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders['orders_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'file_icons/preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $orders['customers_name']; ?></td>
 				<td class="dataTableContent"><?php echo $orders['customers_group_name']; ?></td>
                <td class="dataTableContent" align="right"><?php echo strip_tags($orders['order_total']); ?></td>
                <td class="dataTableContent" align="center"><?php echo tep_datetime_short($orders['date_purchased']); ?></td>
                <td class="dataTableContent" align="right"><?php echo $orders['orders_status_name']; ?></td>
                <td class="dataTableContent" align="right"><?php if (isset($oInfo) && is_object($oInfo) && ($orders['orders_id'] == $oInfo->orders_id)) { echo tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID')) . 'oID=' . $orders['orders_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icons/info_icons/icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
  ...
  switch ($action) {
    ...
    default:
      if (isset($oInfo) && is_object($oInfo)) {
		...
		$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=edit') . '">' . tep_image_button('button_details.png', IMAGE_DETAILS) . '</a> <a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $oInfo->orders_id . '&action=delete') . '">' . tep_image_button('button_delete.png', IMAGE_DELETE) . '</a>');
		...
		$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ORDERS_EDIT, 'oID=' . $oInfo->orders_id) . '">' . tep_image_button('button_edit.png', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_CREATE_ORDER) . '">' . tep_image_button('button_create_order.png', IMAGE_CREATE_ORDER) . '</a>');
		$contents[] = array('text' => '<br>' . TEXT_DATE_ORDER_CREATED . ' ' . tep_date_short($oInfo->date_purchased));
        if (tep_not_null($oInfo->last_modified)) $contents[] = array('text' => TEXT_DATE_ORDER_LAST_MODIFIED . ' ' . tep_date_short($oInfo->last_modified));
        $contents[] = array('text' => '' . TEXT_INFO_PAYMENT_METHOD . ' '  . $oInfo->payment_method);
       	$contents[] = array('text' => '' . TEXT_INFO_CUSTOMER_SERVICE_ID . ' '  . $oInfo->customer_service_id);
      }
      break;
  }
