<?php //$Id: /catalog/product_thumb.php (2226)
...
else
{
	...
	if (is_dir($tmp_path) == false && $tmp_path) mkdir($tmp_path); // JG only do if tmp_path exists

	// Create a new, empty image based on settings:
	if (function_exists('imagecreatetruecolor') && $use_truecolor && $use_resampling /*&& ($image[2] == 2 || $image[2] == 3)*/) // JG allow truecolour for $image[2]==1 (GIF)
	{
	...
	// JG add better transparency support for GIF, and PNG
	if ($image[2] == 1)
	{
	  $tr_idx=imagecolortransparent($src);
	  if ($tr_idx >= 0) // is there a transparent index?
	  {
	    $tr_col = imagecolorsforindex($src, $tr_idx);
	    $tr_idx = imagecolorallocate($tmp_img, $tr_col['red'], $tr_col['green'], $tr_col['blue']);
	    if(!$gif_as_jpeg)
	    {
	      imagefill($tmp_img, 0, 0, $th_bg_color); 
	      imagecolortransparent($tmp_img, $th_bg_color);
//	      imagefill($tmp_img, 0, 0, $tr_idx);
//	      imagecolortransparent($tmp_img, $tr_idx);
	    }
	    else
	    {
	      imagefill($tmp_img, 0, 0, $th_bg_color); 
	      imagecolortransparent($tmp_img, $th_bg_color); // hrm, may not need this line...
	    }
	  }
	}
        else if($image[2] == 3)
	{
            ...
            $tr_col = imagecolorallocatealpha($tmp_img, 0, 0, 0, 127);
            imagefill($tmp_img, 0, 0, $tr_col); // JG need to test with alpha png's
	...
	if (empty($allow_larger) && ($_GET['w'] > $image[0] || $_GET['h'] > $image[1]))
	{
		...
		if($use_truecolor && ($image[2] == 1) && !$gif_as_jpeg) // if GIF input / GIF output, go back to palette
